...
DEFAULT_PROFILE_URL = DEFAULT_POSTER_URL
...
TMDB_SESSION = _build_tmdb_session()
...
def fetch_person_details(id_):
    url = f"{TMDB_BASE_URL}/person/{id_}?api_key={TMDB_API_KEY}"
    try:
        response = TMDB_SESSION.get(url, timeout=10)
        response.raise_for_status()
        data = response.json()
    ...
    if profile_path:
        image_url = "https://image.tmdb.org/t/p/w220_and_h330_face" + profile_path
    else:
        image_url = DEFAULT_PROFILE_URL

    return image_url, biography
